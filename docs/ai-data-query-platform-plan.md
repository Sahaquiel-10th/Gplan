...
## 2026-07：万里牛 AI Skill 接入后的调整

万里牛现已提供官方 AI Skill 和 `hupun-api-cli`。Skill 负责指导 AI 从自然语言选择官方接口和参数，
CLI 负责 App Key / Secret、签名和实际请求。这使平台可以先走一条更短的试点链路：

```text
管理员问题
→ 大模型按万里牛 Skill 选择只读接口
→ hupun-api-cli 查询实时 ERP 数据
→ 将返回结果与个人记忆、百炼知识库 RAG 一起交给大模型
→ 生成问数回答
```

因此，前四层在“AI问数测试”阶段可以暂缓建设，但不能视为永久取消：

- Skill 可以替代一部分数据接入层工作：接口发现、参数说明、鉴权签名和单次实时请求。
- Skill 不能替代业务数据库/数仓：没有历史快照、离线汇总、跨期重算和稳定缓存。
- Skill 不能完全替代指标语义层：销售额、退款率、毛利等口径仍需由公司统一定义。
- Skill 不能解决跨数据源分析：万里牛与支付宝、银行、广告、会员数据的关联仍需平台完成。
- Skill 本身不等于员工权限系统：当前试点仅对管理员开放，并只允许查询/获取类接口。

当前决策是先用官方 Skill 验证真实问题、接口覆盖率和回答质量；当出现高频固定指标、跨源对账、
大数据量分页、性能或权限需求时，再把已验证的场景逐步沉淀到原方案的数据仓库和指标语义层中。

## 对话上下文策略

同一个对话窗口内，AI 应该看到最近若干条真实聊天消息；跨窗口的长期偏好和重要事实，则继续依赖明记忆、暗记忆和企业知识库召回。

当前建议：

```text
system：安全规则 + 模型提示词 + 记忆/RAG 上下文
history：当前对话最近 12 条真实消息
user：当前用户新消息
```
...
- `CHAT_HISTORY_MESSAGES` 控制同一对话传给模型的最近消息条数，建议 10-15，默认 12。
- 12 条大约覆盖最近 6 轮对话，通常比 20 条更稳，成本和上下文污染更可控。
