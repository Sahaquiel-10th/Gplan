...
管理员可在“管理后台 -> AI问数测试”使用独立聊天框。服务端会先用当前模型根据万里牛 AI Skill
选择官方只读接口、生成符合接口文档的参数，并通过 `hupun-api-cli` 查询 ERP；最终回答的上下文为：

```text
系统提示词 + 当前对话 + 管理员个人记忆 + 百炼知识库 RAG + 万里牛 AI Skill 查询结果
```

需要配置 `WANLINIU_APP_KEY`、`WANLINIU_APP_SECRET` 和 `HUPUN_API_CLI_PATH`。凭证只保存在
服务端环境变量中。测试版会拒绝新增、修改、审核、关闭、发货、同步等写接口，只开放官方索引里的
查询/获取类接口。
